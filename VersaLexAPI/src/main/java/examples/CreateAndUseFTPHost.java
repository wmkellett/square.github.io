 ...
 * - How to create a temporary FTP host, configure it, and use a mailbox controller
 *   to send a test file
 ******************************************************************************/
public class CreateAndUseFTPHost {
  public static void main(String[] args)
  {
    new CreateAndUseFTPHost();
  }
  public CreateAndUseFTPHost() {
...
/*------------------------------------------------------------------------------
 *    Create and configure a temporary FTP trading partner (i.e. host/mailbox)
 *----------------------------------------------------------------------------*/
      String host = lexicom.activateHost("Generic FTPs", "FTP", false); // "FTP" is the preferred alias;
      ...
      lexicom.setProperty(lexicom.HOST, path, "Port", "21");
//      lexicom.setProperty(lexicom.HOST, path, "Securitymode", "None");
//      lexicom.setProperty(lexicom.HOST, path, "Securitymode", "SSL Implicit");
      lexicom.setProperty(lexicom.HOST, path, "Securitymode", "SSL AUTH");
//      lexicom.setProperty(lexicom.HOST, path, "Datatype", "Binary");
      lexicom.setProperty(lexicom.HOST, path, "Datatype", "ASCII");
//      lexicom.setProperty(lexicom.HOST, path, "Datachannelmode", "Active");
      lexicom.setProperty(lexicom.HOST, path, "Datachannelmode", "Passive");
      lexicom.setProperty(lexicom.HOST, path, "Substitutepassiveipaddress", "True");

      lexicom.setProperty(lexicom.MAILBOX, path, "Username", "cleoftptest");
      lexicom.setProperty(lexicom.MAILBOX, path, "Password", "*ExoHFhwQEg**"); // password encoded here
 ...
 *----------------------------------------------------------------------------*/
      ...
      for (int i = 0; success && i < outgoing.length; i++)
        // last parameter is keepalive; keepalive until last file so same FTP session used for both files
